...
void _destruir_elem(tipoelem *E) {
    if(E->compLexico == LIB) dlclose(E->valor.funptr);
    free(E->lexema);
}
...
/* Funcion que modifica el valor de una variable de un nodo del arbol*/
/* Se presupone que existe un nodo con la clave indicada*/
void modificar_nodo(abb *A, tipoclave cl, double valor) {
    if (es_vacio(*A)) {
        return;
    }
    int comp = _comparar_clave_elem(cl, (*A)->info);
